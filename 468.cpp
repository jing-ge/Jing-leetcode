...
public:
    ...
    string validIPAddress(string IP) {
        ...
        if(ip.size()==4)return ipv4(ip);
        ip.clear();
        ip = split(IP,':');
        if(ip.size()==8)return ipv6(ip);
        else return "Neither";
    }
    string ipv4(vector<string> &ip){
        for(string &s:ip){
            ...
            if(s[0]=='0'&&s.size()!=1||stoi(s)>255)return "Neither";
        }
        return "IPv4";
    }
    string ipv6(vector<string> &ip){
        for(string &s:ip){
            ...
            for(char &c:s){
                if(set.count(c)==0)return "Neither";
            }
        }
        return "IPv6";
    }
...
int main(){
    Solution s;
    string ip = "1.0.1.";
    string res = s.validIPAddress(ip);
    cout<<res<<endl;
    return 0;
}